...
async def _run_pipeline(record: dict):
    ...
    try:
        ...
        if raw_call_type not in ("process_call", "offer_call"):
            logger.info(
                "Skipping pipeline — call type '%s' is not process or offer | call_id=%s",
                raw_call_type, call_id,
            )
            return

        # Skip process calls under 3 minutes
